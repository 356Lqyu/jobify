        ...
        if (widget.existingJob != null) {
          ...
          if (targetTypeId != null && targetTypeId.isNotEmpty) {
            ...
            if (_selectedJobType == null) {
              ...
              print('ound job type: ${_selectedJobType!['name']} (ID: ${_selectedJobType!['job_type_id']})');
            }
          } else {
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              _buildTextField(controller: _videoUrlController, label: 'Video URL', hintText: 'Paste YouTube or Vimeo link (optional)', validator: (value) {
                if (value == null || value.isEmpty) return null;
              ...
              const SizedBox(height: 16),
              _buildTextField(controller: _jobTitleController, label: 'Job Title *', hintText: 'e.g. Senior Flutter Developer', validator: (v) => v?.trim().isEmpty == true ? 'Required' : null),
              const SizedBox(height: 16),
              _buildTextField(controller: _descriptionController, label: 'Job Description *', hintText: 'Describe the role, responsibilities, and requirements...', maxLines: 5, validator: (v) => v?.trim().isEmpty == true ? 'Required' : null),
              const SizedBox(height: 16),
              Row(
                children: [
                  Expanded(child: _buildTextField(controller: _minSalaryController, label: 'Min Salary (MYR)', hintText: 'e.g. 3000', keyboardType: TextInputType.number, validator: _validateSalaryMin, onChanged: (_) => _formKey.currentState?.validate())),
                  const SizedBox(width: 12),
                  Expanded(child: _buildTextField(controller: _maxSalaryController, label: 'Max Salary (MYR)', hintText: 'e.g. 5000', keyboardType: TextInputType.number, validator: _validateSalaryMax)),
                ],
              ),
              const SizedBox(height: 16),
                ...
                items: [
                  ...
                  ..._branches.map((branch) {
                    ...
                    String label = branchName.isNotEmpty
                        ? (address.isNotEmpty ? '$branchName ($address)' : branchName)
              ...
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedCategory = v; _formData['job_category_id'] = v?['job_category_id']; }),
              ...
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedJobType = v; _formData['job_type_id'] = v?['job_type_id']; }),
                validator: (v) => v == null ? 'Required' : null,
              ),
              const SizedBox(height: 16),
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedExpLevel = v; _formData['experience_level_id'] = v?['experience_level_id']; }),
                validator: (v) => v == null ? 'Required' : null,
              ),
              const SizedBox(height: 16),
              Row(children: [Checkbox(value: _formData['remote_option'], onChanged: (v) => setState(() => _formData['remote_option'] = v ?? false)), const Text('Remote option available')]),
              const SizedBox(height: 16),
              _buildTextField(controller: _vacancyController, label: 'Number of vacancies', hintText: 'e.g. 3', keyboardType: TextInputType.number, validator: _validateVacancyCount),
              const SizedBox(height: 16),
              ListTile(
                contentPadding: EdgeInsets.zero,
                title: const Text('Application Deadline'),
                subtitle: Text(_formData['application_deadline'] != null ? _formData['application_deadline'].toString().split('T')[0] : 'Not set'),
                trailing: const Icon(Icons.calendar_today),
                onTap: () async {
                  final date = await showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime.now(), lastDate: DateTime.now().add(const Duration(days: 365)));
                  if (date != null) setState(() => _formData['application_deadline'] = date.toIso8601String());
                },
              ),
