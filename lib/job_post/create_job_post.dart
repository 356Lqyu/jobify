...
class _CreateJobPostState extends State<CreateJobPost> {
  ...
  static const List<String> allowedImageExtensions = ['png', 'jpg', 'jpeg', 'webp'];
  ...
  void _initFormData() {
    ...
    if (widget.existingJob != null) {
      ...
      print('Raw experience_level_id: ${widget.existingJob!['experience_level_id']}');
      ...
      print('Extracted experience_level_id: ${_formData['experience_level_id']}');

      final branchField = widget.existingJob!['branch_id'];
      String? branchIdString;
      if (branchField != null) {
        if (branchField is Map) {
          branchIdString = (branchField['branch_id'] ?? branchField['id'])?.toString();
        } else {
  ...
  Future<void> _loadUserAndReferences() async {
    try {
      ...
      setState(() {
        ...
        print('Categories: ${categories.map((c) => '${c['job_category_id']}: ${c['name']}')}');
        print('Job Types: ${types.map((t) => '${t['job_type_id']}: ${t['name']}')}');
        print('Experience Levels: ${levels.map((l) => '${l['experience_level_id']}: ${l['name']}')}');
        print('Branches: ${branches.map((b) => '${b['branch_id']}: ${b['branch_name']}')}');
        ...
        if (widget.existingJob != null) {
          ...
          if (targetCatId != null && targetCatId.isNotEmpty) {
            ...
            if (_selectedCategory == null) {
              ...
              print('Found category: ${_selectedCategory!['name']} (ID: ${_selectedCategory!['job_category_id']})');
            }
          } else {
          ...
          if (targetTypeId != null && targetTypeId.isNotEmpty) {
            ...
            if (_selectedJobType == null) {
              ...
              print('Found job type: ${_selectedJobType!['name']} (ID: ${_selectedJobType!['job_type_id']})');
            }
          } else {
            print('No job type ID to search for');
          }

          final targetLevelId = _formData['experience_level_id']?.toString().trim();
          ...
          if (targetLevelId != null && targetLevelId.isNotEmpty) {
            ...
            if (_selectedExpLevel == null) {
              ...
              print('Found experience level: ${_selectedExpLevel!['name']} (ID: ${_selectedExpLevel!['experience_level_id']})');
            }
          } else {
          ...
          if (_selectedBranchId != null) {
            final branchExists = branches.any((b) => b['branch_id'].toString() == _selectedBranchId);
            if (!branchExists) {
              print('Selected branch ID $_selectedBranchId not found in loaded branches');
              _selectedBranchId = null;
            } else {
  ...
  Future<void> _saveDraft() async {
    ...
    final draft = {
      ...
      'salary_min': _minSalaryController.text,
      'salary_max': _maxSalaryController.text,
      'vacancy_count': _vacancyController.text,
      'video_url': _videoUrlController.text,
      'location': _selectedBranchId ?? _customLocationValue,
      'category_id': _selectedCategory?['job_category_id'],
      'type_id': _selectedJobType?['job_type_id'],
      'level_id': _selectedExpLevel?['experience_level_id'],
      'remote_option': _formData['remote_option'],
      'deadline': _formData['application_deadline'],
    };
    await prefs.setString('job_draft', jsonEncode(draft));
  }

  Future<void> _loadDraft() async {
    if (widget.existingJob != null) return;
    final prefs = await SharedPreferences.getInstance();
    final draftStr = prefs.getString('job_draft');
    if (draftStr == null) return;
    try {
      final draft = jsonDecode(draftStr) as Map<String, dynamic>;
      _jobTitleController.text = draft['job_title'] ?? '';
      _descriptionController.text = draft['description'] ?? '';
      _minSalaryController.text = draft['salary_min'] ?? '';
      _maxSalaryController.text = draft['salary_max'] ?? '';
      _vacancyController.text = draft['vacancy_count'] ?? '1';
      _videoUrlController.text = draft['video_url'] ?? '';
      if (draft['location'] == 'Remote' || draft['location'] == 'Hybrid') {
        _customLocationValue = draft['location'];
        _selectedBranchId = null;
      } else {
        _selectedBranchId = draft['location'];
        _customLocationValue = null;
      }
      setState(() {
        _formData['remote_option'] = draft['remote_option'] ?? false;
        _formData['application_deadline'] = draft['deadline'];
      });
    } catch (_) {}
  }
  ...
  Future<void> _submit() async {
    ...
    if (_selectedBranchId != null) {
      ...
      if (finalLocation.isEmpty) finalLocation = selectedBranch['branch_name'] ?? 'Branch location';
    ...
    if (_userId == null || _companyProfile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Missing user or company profile.'), backgroundColor: Colors.red),
      );
    ...
    if (_formData['job_category_id'] == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select all required dropdown fields.'), backgroundColor: Colors.red),
      );
      setState(() => _isLoading = false);
      return;
    }
    ...
    try {
      newImageUrls = await _uploadImages().timeout(
        const Duration(seconds: 90),
        onTimeout: () => throw Exception('Image upload timed out (over 90 seconds)'),
      );
    } catch (e) {
      print('Image upload failed: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Image upload error: ${e.toString()}'), backgroundColor: Colors.red, duration: const Duration(seconds: 4)),
      );
      setState(() => _isLoading = false);
      return;
    }
    ...
    try {
      if (widget.existingJob != null) {
        ...
        final updatedCount = await _jobRepo.updateJobPost(jobId, data).timeout(
          const Duration(seconds: 30),
          onTimeout: () => throw Exception('Update timed out'),
        );
        if (updatedCount == 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Update failed: job not found.'), backgroundColor: Colors.red),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        final jobId = await _jobRepo.createJobPost(data, autoCreateSocialPost: true).timeout(
          const Duration(seconds: 45),
          onTimeout: () => throw Exception('Job creation timed out'),
        );
        if (jobId != null) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Job published and shared to feed'), backgroundColor: Colors.green),
          );
          final prefs = await SharedPreferences.getInstance();
          await prefs.remove('job_draft');
          _resetForm();
        } else {
          throw Exception('createJobPost returned null');
        }
      }
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  const Text('Create Job Post', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const Spacer(),
                  ElevatedButton(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16),
              child: _selectedPostType == 1 ? _buildJobPostForm(totalImages, canAddMore) : _buildCompanyUpdateSection(),
            ),
          ),
        ],
      ),
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: isSelected ? Colors.blue : Colors.grey.shade100, borderRadius: BorderRadius.circular(30)),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    if (_userId == null || _companyProfile == null) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('User or company profile not loaded.'), backgroundColor: Colors.red));
                      return;
                    }
                    final result = await showModalBottomSheet<bool>(
                      ...
                      builder: (_) => ChangeNotifierProvider(
                        create: (_) => SocialFeedProvider(repository: FeedRepository(), userId: _userId!),
                    ...
                    if (result == true && widget.onPostSuccess != null) widget.onPostSuccess!();
                  },
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ..._imageFiles.map((file) => Stack(
                    children: [
                      ...
                      Positioned(right: 0, top: 0, child: InkWell(onTap: () => setState(() => _imageFiles.remove(file)), child: const Icon(Icons.cancel, color: Colors.red, size: 20))),
                    ],
                  )),
                  ..._existingImageUrls.map((url) => Stack(
                    children: [
                      ...
                      Positioned(right: 0, top: 0, child: InkWell(onTap: () => setState(() => _existingImageUrls.remove(url)), child: const Icon(Icons.cancel, color: Colors.red, size: 20))),
                    ],
                  )),
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        if (result != null) {
                          ...
                          if (tooLarge) {
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Each image must be <5MB'), backgroundColor: Colors.red));
                            return;
                          }
                          if (totalImages + newFiles.length <= 5) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Maximum 5 images allowed'), backgroundColor: Colors.red));
                          }
                        }
                      },
                      child: Container(
              ...
              const SizedBox(height: 24),
              _buildTextField(controller: _videoUrlController, label: 'Video URL', hintText: 'Paste YouTube or Video link (optional)', validator: (value) {
                if (value == null || value.isEmpty) return null;
              ...
              const SizedBox(height: 16),
              _buildTextField(controller: _jobTitleController, label: 'Job Title *', hintText: 'e.g. Senior Flutter Developer', validator: (v) => v?.trim().isEmpty == true ? 'Required' : null, onChanged: (_) => _saveDraft()),
              const SizedBox(height: 16),
              _buildTextField(
                ...
                maxLength: 5000,   // character limit and counter
              ),
              const SizedBox(height: 16),
              Row(
                children: [
                  Expanded(child: _buildTextField(controller: _minSalaryController, label: 'Min Salary (MYR)', hintText: 'e.g. 3000', keyboardType: TextInputType.number, validator: _validateSalaryMin, onChanged: (_) { _formKey.currentState?.validate(); _saveDraft(); })),
                  const SizedBox(width: 12),
                  Expanded(child: _buildTextField(controller: _maxSalaryController, label: 'Max Salary (MYR)', hintText: 'e.g. 5000', keyboardType: TextInputType.number, validator: _validateSalaryMax, onChanged: (_) => _saveDraft())),
                ],
              ),
              const SizedBox(height: 16),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      ...
                      isExpanded: true,
                      items: [
                        const DropdownMenuItem<String>(value: 'Remote', child: Text('Remote')),
                        const DropdownMenuItem<String>(value: 'Hybrid', child: Text('Hybrid')),
                        const DropdownMenuItem<String>(value: '---', enabled: false, child: Divider()),
                        ..._branches.map((branch) {
                          ...
                          String label = branchName.isNotEmpty ? (address.isNotEmpty ? '$branchName ($address)' : branchName) : (address.isNotEmpty ? address : 'Unnamed branch');
                          if (label.length > 45) label = '${label.substring(0, 42)}...';
                          return DropdownMenuItem<String>(value: branch['branch_id'], child: Text(label, overflow: TextOverflow.ellipsis));
                        }).toList(),
                      ],
                      onChanged: (value) {
                        setState(() {
                          if (value == 'Remote' || value == 'Hybrid') {
                            ...
                            // Auto-check remote option
                            _formData['remote_option'] = true;
                          } else if (value != null && value != '---') {
                            _selectedBranchId = value;
                            _customLocationValue = null;
                            // Uncheck remote option for branch locations
                            _formData['remote_option'] = false;
                          }
                          _saveDraft();
                        });
                      },
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      ...
                      final fresh = await _jobRepo.fetchCompanyBranches(_companyProfile!['company_id'], forceRefresh: true);
                      setState(() => _branches = fresh);
                    },
                  ),
                ],
              ),
              ...
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedCategory = v; _formData['job_category_id'] = v?['job_category_id']; _saveDraft(); }),
              ...
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedJobType = v; _formData['job_type_id'] = v?['job_type_id']; _saveDraft(); }),
                validator: (v) => v == null ? 'Required' : null,
              ),
              const SizedBox(height: 16),
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                onChanged: (v) => setState(() { _selectedExpLevel = v; _formData['experience_level_id'] = v?['experience_level_id']; _saveDraft(); }),
                validator: (v) => v == null ? 'Required' : null,
              ),
              const SizedBox(height: 16),
              Row(
                children: [
                  Checkbox(
                    ...
                    onChanged: (v) => setState(() { _formData['remote_option'] = v ?? false; _saveDraft(); }),
                  ),
                  const Text('Remote option available'),
                ],
              ),
              const SizedBox(height: 16),
              _buildTextField(controller: _vacancyController, label: 'Number of vacancies', hintText: 'e.g. 3', keyboardType: TextInputType.number, validator: _validateVacancyCount, onChanged: (_) => _saveDraft()),
              const SizedBox(height: 16),
              ListTile(
                ...
                subtitle: Text(
                  _formData['application_deadline'] != null
                      ? DateFormat('dd MMM yyyy, hh:mm a').format(DateTime.parse(_formData['application_deadline']))
                      : 'Not set',
                ),
                ...
                onTap: () async {
                  ...
                  if (date != null) {
                    ...
                    if (time != null) {
                      ...
                      setState(() => _formData['application_deadline'] = dateTime.toIso8601String());
                      _saveDraft();
                    }
                  }
                },
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : Text(widget.existingJob == null ? 'Publish Job' : 'Save Changes'),
                ),
              ),
            ],
          ),
  ...
  InputDecoration _inputDecoration(String label, {String? hintText}) {
    return InputDecoration(
      ...
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey.shade500)),
