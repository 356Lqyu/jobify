...
class _SettingPageState extends State<SettingPage> {
  ...
  Future<void> _fetchJobPostCount() async {
    final userId = supabase.auth.currentUser?.id;
    if (userId == null || role?.toUpperCase() != 'POSTER') return;

    try {
      final List<dynamic> jobs = await supabase
          .from('job_post')
          .select('job_id') // fetch only IDs
          .eq('created_by', userId);

      final totalJobs = jobs.length; // count from list length
      if (mounted) setState(() => _jobPostCount = totalJobs);
    } catch (e) {
      if (mounted) setState(() => _jobPostCount = 0);
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
              buildStatItem(_jobPostCount.toString(), 'Job Posts'),
